...
Пример конфирурации второго узла кластерного приложения TcpSwitch - [avalanche-tcpswitch-Y-config.xml](conf/avalanche-tcpswitch-Y-config.xml)
...
В каталоге [log](log) опубликован франмент лога обмена между клиентом (kafka-console-producer) и кластером kafka через
